...
import { AlertTriangle, TrendingUp, RefreshCcw, Clock } from "lucide-react"
...
export function PredictiveAnalytics({ sensorData, channelId, location }: PredictiveAnalyticsProps) {
  ...
  const generatePredictions = () => {
    ...
    try {
      ...
      if (latestPrediction.predictions.co > 8) {
        issues.push({
          issue: "Potential CO level increase above safe threshold",
          confidence: 0.85,
          timestamp: latestPrediction.timestamp,
        })
      }

      if (latestPrediction.predictions.pm2_5 > 25) {
        issues.push({
          issue: "Air quality levels approaching unhealthy range",
          confidence: 0.78,
          timestamp: latestPrediction.timestamp,
        })
      }

      if (latestPrediction.predictions.temperature > 35) {
        issues.push({
          issue: "High temperature conditions predicted",
          confidence: 0.82,
          timestamp: latestPrediction.timestamp,
        })
      }

      if (latestPrediction.predictions.humidity < 30) {
        issues.push({
          issue: "Low humidity conditions may increase particulate matter",
          confidence: 0.75,
          timestamp: latestPrediction.timestamp,
        })
      }
  ...
  const predictionChartData = predictions.map((p) => ({
    ...
    temperature: p.predictions.temperature.toFixed(1),
    humidity: p.predictions.humidity.toFixed(1),
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <Select value={selectedMetric} onValueChange={setSelectedMetric}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="pm25">Air Quality </SelectItem>
                ...
                <LineChart data={predictionChartData}>
                  ...
                  {selectedMetric === "pm25" && (
                    <Line
                      type="monotone"
                      dataKey="pm25"
                      name="Air Quality (PM2.5 equivalent)"
